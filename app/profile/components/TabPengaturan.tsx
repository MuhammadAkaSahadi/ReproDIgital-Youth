...
import { uploadAvatarServer, deleteAccountServer } from "../actions";
...
export function TabPengaturan({ profile }: { profile?: any }) {
  ...
  const securityForm = useForm<SecuritySchema>({
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(
        `Gagal mengunggah foto profil: ${err.message || "Unknown error"}`,
      );
    }
  ...
  const onSecuritySubmit = async (data: SecuritySchema) => {
  ...
  const onDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      // 1. Memanggil Server Action karena Supabase melarang deleteUser dari public client
      await deleteAccountServer(profile.id);
      
      // 2. Membersihkan sesi lokal di browser (storage/cookies)
      const supabase = createClient();
      await supabase.auth.signOut();
      
      toast.success("Akun Anda telah berhasil dihapus secara permanen.");
      
      // 3. Mengarahkan kembali ke halaman login
      window.location.href = "/login";
    } catch (error: any) {
      toast.error(`Gagal menghapus akun: ${error.message || "Kesalahan internal."}`);
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-4xl space-y-12 animate-in fade-in duration-500">
      ...
      <section>
        <h3 className="text-xl font-heading font-bold text-gray-900 border-b border-gray-200 pb-4 mb-6">
          Edit Profil
        </h3>
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="w-24 h-24 rounded-full bg-gray-200 border-2 border-gray-200 overflow-hidden relative group">
              <img
                src={
                  profile?.avatar_url ||
                  `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.full_name || "User")}&background=0D9488&color=fff&size=400`
                }
                alt="Avatar"
              ...
              <Label
                ...
                className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center cursor-pointer"
              >
                {isUploadingAvatar ? (
            ...
            <div className="flex flex-col gap-2 w-full">
              <Label
                htmlFor="avatar-upload"
                className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-950 disabled:pointer-events-none disabled:opacity-50 border h-9 px-3 w-full text-teal-700 border-teal-600 hover:bg-teal-50 cursor-pointer"
              >
          ...
          >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-11 pl-8 focus-visible:ring-teal-500"
                  />
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-11 focus-visible:ring-teal-500 text-gray-700"
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Lulus/Lainnya">
                      Lulus / Lainnya
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            <div className="space-y-2">
              ...
              {profile?.role !== "counselor" && (
                <p className="text-gray-500 text-xs mt-1">
                  Status Anda tidak memerlukan spesialisasi.
                </p>
              )}
            </div>
            ...
            <div className="pt-4 flex gap-3">
              <Button
                ...
                className="bg-coral-500 hover:bg-coral-600 text-white min-w-[120px]"
              >
                {isProfileLoading ? (
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                className="h-11 pr-10 focus-visible:ring-teal-500"
              />
              <button
                type="button"
                onClick={() => setShowCurrentPassword(!showCurrentPassword)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-teal-600 transition-colors focus:outline-none rounded-md"
              >
                {showCurrentPassword ? (
      ...
      {/* SECTION 2.5: Logout (Mobile Only) */}
      <section className="md:hidden">
        <Button
          ...
          className="w-full h-12 flex items-center justify-center border-gray-300 text-gray-700 hover:bg-gray-50"
        >
        ...
        <div className="mt-12 bg-red-50 border-2 border-red-200 rounded-xl p-6 md:p-8 flex items-center justify-between gap-6 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-[20px] font-heading font-bold text-red-600">
                Hapus Akun Permanen
              </h3>
            </div>
            <p className="text-gray-600 text-sm max-w-lg mb-0">
              Perhatian: Begitu Anda menghapus akun ini, seluruh data progres,
              kuis, dan artikel yang tersimpan tidak akan bisa dipulihkan
              kembali. Lakukan dengan hati-hati.
            </p>
          </div>
          ...
          <Dialog>
            <DialogTrigger
              render={
                <Button
                  variant="destructive"
                  className="bg-red-600 hover:bg-red-700 text-white h-12 px-6 shadow-sm"
                />
              }
            >
            ...
            <DialogContent className="sm:max-w-[480px]">
              <DialogHeader>
                ...
                <DialogDescription className="pt-4 text-black text-sm">
                  Aksi ini tidak bisa dibatalkan. Ini akan secara permanen
                  menghapus akun <strong>@{profile?.username || "user"}</strong>{" "}
              ...
              <div className="my-4">
                <Label className="text-gray-700 mb-2 block text-sm">
                  Untuk mengkonfirmasi, ketik kata <strong>HAPUS AKUN</strong>{" "}
                  di bawah ini:
                </Label>
                <Input
                  value={deleteInput}
